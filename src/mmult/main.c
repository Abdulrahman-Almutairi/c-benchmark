...
int main(int argc, char** argv)
{
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (strcmp(argv[i], "-B") == 0 || strcmp(argv[i], "-b") == 0) {
      assert (++i < argc);
      char *dim = argv[i];
      sscanf(dim, "%dx%d", &B_rows, &B_cols);

      continue;
    }

    /* Dimensions of blocking technique */
    if (strcmp(argv[i], "-D") == 0 || strcmp(argv[i], "-d") == 0) {
      assert (++i < argc);
      dim_blocking = atoi(argv[i]);
  ...
  /* Handle condition for matrix multiplication */
  if (A_cols != B_rows) {
  ...
  if (help || impl == NULL) {
    ...
    printf("    -a | -A          Set the number of rows and columns (nxm) of matrix A (default = 50x50)\n");
    printf("    -b | -B          Set the number of rows and columns (nxm) of matrix B (default = 50x50)\n");
    printf("    -d | -D          Set the dimensions (bxb) of the blocking submatrix b (default = 5)\n");
  ...
  // byte* matrix_A   = __ALLOC_INIT_DATA(byte, A_rows * A_cols);
  // byte* matrix_B   = __ALLOC_INIT_DATA(byte, B_rows * B_cols);
  // byte* ref        = __ALLOC_INIT_DATA(byte, A_cols * B_rows);
  // byte* dest       = __ALLOC_DATA     (byte, A_cols * B_rows);

  float* matrix_A   = __ALLOC_INIT_FLOAT_DATA(A_rows * A_cols);
  float* matrix_B   = __ALLOC_INIT_FLOAT_DATA(B_rows * B_cols);
  float* ref        = __ALLOC_INIT_FLOAT_DATA(A_cols * B_rows);
  float* dest       = __ALLOC_FLOAT_DATA     (A_cols * B_rows);

  /* Setting a guards, which is 0xdeadcafe.
     The guard should not change or be touched. */
  __SET_GUARD(ref , A_cols * B_rows);
  __SET_GUARD(dest, A_cols * B_rows);
  ...
  args_ref.inputA    = matrix_A;
  args_ref.inputB    = matrix_B;
  args_ref.output    = ref;

  args_ref.A_rows    = A_rows;
  args_ref.A_cols    = A_cols;
  args_ref.B_cols    = B_cols;

  args_ref.dim_blocking = dim_blocking;
  ...
  args.size      = data_size;
  args.inputA    = matrix_A;
  args.inputB    = matrix_B;
  args.output    = dest;
  ...
  args.dim_blocking = dim_blocking;
  ...
  bool match = __CHECK_MATCH(ref, dest, A_cols * B_rows);
  bool guard = __CHECK_GUARD(     dest, A_cols * B_rows);
  if (match && guard) {
    printf("Success\n");
  } else if (!match && guard) {
